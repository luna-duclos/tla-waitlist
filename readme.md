...
This readme was originally written using Ubuntu but it should cover the basics for most Linux distros, if you haven't got WSL setup see [Installing WSL | Microsoft Docs](https://docs.microsoft.com/en-us/windows/wsl/install).

Instructions to install directly on windows with simplified instructions are also provided
...
[HeidiSQL](https://www.heidisql.com/) (optional to easily edit & view DB)

[Git](https://git-scm.com/install/windows) (to run bash scripts)  
[RustRover](https://www.jetbrains.com/rust/)  
[Node](https://nodejs.org/en/download)

_You will also need [Visual studio build tools](https://visualstudio.microsoft.com/downloads/?q=build+tools) with 'Desktop development with c++' checked._
...
| [`sse/`](./tree/main/sse) | Rust | A service that broadcasts server driven events to UI clients |

###### Setup and run the SSE Server

1. Generate a secret key using `openssl rand -hex 32` and copy it somewhere safe. This key is needed to start the SSE server and the backend process
2. Start the docker image. **You must** pass the secret key from step three as an environment variable called `SSE_SECRET`
...
1. Open the sse folder using rustrover
2. Open cargo.toml to make sure it is detected. Make sure rustrover has a rust toolchain, it should prompt you.
3. Click edit configuration & add environment variable SSE_SECRET
4. Run SSE server
...
3. Run the `download_convert_sde_to_sqlite.sh` script
...
<details>
   ...
   sh scripts/download_convert_sde_to_sqlite.sh
...
**Windows**
1. Open the backend folder using RustRover & open the file cargo.toml
2. Make sure you've followed setup instructions for config.toml
3. Edit/create a new run configuration (cargo) & add the DATABASE_URL environment variable the value should be the same in your config.toml
4. Generate the database using either HeidiSQL
- (Open mysql db)
- (file => load sql file => run  )
- Tools => user manager => create one for backend & add to config.toml
- You can run the script to add yourself as admin later
5. Open program git bash, navigate to backend/scripts & run `bash download_convert_sde_to_sqlite.sh`
6. Run the backend
