...
export function SRPReportDetail() {
  ...
  const [killmailData] = useApi(reportId ? `/api/admin/srp/reports/${reportId}/killmail/enriched` : null);
  const [fleetValidation, fleetValidationError] = useApi(reportId ? `/api/admin/srp/reports/${reportId}/fleet-validation` : null);
  const [srpValidation, srpValidationError] = useApi(reportId ? `/api/admin/srp/reports/${reportId}/srp-validation` : null);
  ...
  const handleCalculateAppraisal = React.useCallback(async (appraisalType = 'auto') => {
            ...
            <table style={{ width: "100%" }}>
              <tbody>
                ...
                <tr>
                  ...
                  <td style={{ padding: "0.5em 0" }}>{formatDatetime(new Date(report.submitted_at * 1000))}</td>
                </tr>
                <tr>
                  ...
                  <td style={{ padding: "0.5em 0" }}>
                    <span
                      style={{
                        ...
                        borderRadius: "3px",
                        backgroundColor: 
                          report.status === "pending" ? "#fff3cd" :
                          report.status === "approved" ? "#d4edda" :
                          report.status === "rejected" ? "#f8d7da" :
                          report.status === "paid" ? "#cce5ff" : "#6c757d",
                        color: 
                          report.status === "pending" ? "#856404" :
                          report.status === "approved" ? "#155724" :
                          report.status === "rejected" ? "#721c24" :
                          report.status === "paid" ? "#004085" : "#ffffff",
                      }}
                    >
            ...
            <table style={{ width: "100%" }}>
              <tbody>
                ...
                <tr>
                  ...
                  <td style={{ padding: "0.5em 0" }}>
                    {report.payout_date ? formatDatetime(new Date(report.payout_date * 1000)) : "N/A"}
                  </td>
                </tr>
                <tr>
        ...
        <div style={{ marginTop: "2em" }}>
          ...
          <div style={{ 
            ...
            {fleetValidationError && typeof fleetValidationError !== 'function' ? (
              <div style={{ color: "#721c24", backgroundColor: "#f8d7da", padding: "0.5em", borderRadius: "4px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1em" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "1em" }}>
                  <span style={{ fontWeight: "bold", color: "black" }}>Fleet Membership at Death:</span>
                  <span style={{
                ...
                {srpValidationError && typeof srpValidationError !== 'function' ? (
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "1em" }}>
                    <span style={{ fontWeight: "bold", color: "black" }}>SRP Coverage at Death:</span>
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.5em" }}>
                      <span style={{
                        padding: "0.25em 0.75em",
                        borderRadius: "20px",
                        backgroundColor: srpValidation.alt_needs_linking ? "#fff3cd" : (srpValidation.had_srp_coverage ? "#d4edda" : "#f8d7da"),
                        color: srpValidation.alt_needs_linking ? "#856404" : (srpValidation.had_srp_coverage ? "#155724" : "#721c24"),
                        fontWeight: "bold"
                      }}>
                        {srpValidation.alt_needs_linking ? "⚠ ALT NEEDS LINKING" : (srpValidation.had_srp_coverage ? "✓ HAD COVERAGE" : "✗ NO COVERAGE")}
                      </span>
                      {srpValidation.had_srp_coverage && srpValidation.payment_date && srpValidation.payment_amount && (
                        <div style={{ fontSize: "0.9em", color: "#6c757d" }}>
                          {srpValidation.coverage_character && srpValidation.coverage_type && (
                            <div style={{ fontWeight: "bold", marginBottom: "0.25em" }}>
                              ...
                              {srpValidation.coverage_type === "main" && `Coverage via main: ${srpValidation.coverage_character}`}
                              {srpValidation.coverage_type === "alt" && `Coverage via alt: ${srpValidation.coverage_character}`}
                            </div>
                          )}
                          <div>Payment: {srpValidation.payment_amount.toLocaleString()} ISK</div>
                          <div>Date: {srpValidation.payment_date}</div>
                          {srpValidation.expires_at && <div>Expires: {srpValidation.expires_at}</div>}
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2em' }}>
                <div>
                  ...
                  <table style={{ width: '100%' }}>
                    <tbody>
                      <tr>
                        <td style={{ fontWeight: 'bold', padding: '0.5em 0', width: '40%' }}>Character:</td>
                        <td style={{ padding: '0.5em 0', color: 'white' }}>
                          {killmailData.victim.character_name || killmailData.victim.character_id || 'N/A'}
                          {killmailData.victim.character_id && (
                            <span style={{ fontSize: '0.9em' }}> (ID: {killmailData.victim.character_id})</span>
                          )}
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td style={{ padding: '0.5em 0', color: 'white' }}>
                          {killmailData.victim.corporation_name || killmailData.victim.corporation_id || 'N/A'}
                          {killmailData.victim.corporation_id && (
                            <span style={{ fontSize: '0.9em' }}> (ID: {killmailData.victim.corporation_id})</span>
                          )}
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td style={{ padding: '0.5em 0', color: 'white' }}>
                          {killmailData.victim.alliance_name || killmailData.victim.alliance_id || 'N/A'}
                          {killmailData.victim.alliance_id && (
                            <span style={{ fontSize: '0.9em' }}> (ID: {killmailData.victim.alliance_id})</span>
                          )}
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td style={{ padding: '0.5em 0', color: 'white' }}>
                          ...
                          <span style={{ fontSize: '0.9em' }}> (ID: {killmailData.victim.ship_type_id})</span>
                        </td>
                      </tr>
                      <tr>
                        <td style={{ fontWeight: 'bold', padding: '0.5em 0' }}>Damage Taken:</td>
                        <td style={{ padding: '0.5em 0', color: 'white' }}>{killmailData.victim.damage_taken.toLocaleString()}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                ...
                <div>
                  ...
                  <table style={{ width: '100%' }}>
                    <tbody>
                      <tr>
                        <td style={{ fontWeight: 'bold', padding: '0.5em 0', width: '40%' }}>Killmail ID:</td>
                        <td style={{ padding: '0.5em 0', color: 'white' }}>{killmailData.killmail_id}</td>
                      </tr>
                      <tr>
                        <td style={{ fontWeight: 'bold', padding: '0.5em 0' }}>Kill Time:</td>
                        <td style={{ padding: '0.5em 0', color: 'white' }}>{killmailData.killmail_time}</td>
                      </tr>
                      <tr>
                        <td style={{ fontWeight: 'bold', padding: '0.5em 0' }}>Solar System:</td>
                        <td style={{ padding: '0.5em 0', color: 'white' }}>
                          {killmailData.solar_system_name || 'Unknown'}
                          <span style={{ fontSize: '0.9em' }}> (ID: {killmailData.solar_system_id})</span>
                        </td>
                      </tr>
                      <tr>
                        <td style={{ fontWeight: 'bold', padding: '0.5em 0' }}>Moon ID:</td>
                        <td style={{ padding: '0.5em 0', color: 'white' }}>{killmailData.moon_id || 'N/A'}</td>
                      </tr>
                      <tr>
                        <td style={{ fontWeight: 'bold', padding: '0.5em 0' }}>War ID:</td>
                        <td style={{ padding: '0.5em 0', color: 'white' }}>{killmailData.war_id || 'N/A'}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                  ...
                  <div style={{ 
                    ...
                    {abyssalModules.map((module, index) => (
                      <div key={index} style={{ 
                        ...
                        alignItems: 'center',
                        padding: '0.5em 0', 
                        borderBottom: index < abyssalModules.length - 1 ? '1px solid #e9ecef' : 'none',
                        fontSize: '0.9em'
                      }}>
                        <img 
                          src={`https://images.evetech.net/types/${module.typeId || 0}/icon?size=32`}
                          alt={module.name}
                          style={{ 
                            width: '24px', 
                            height: '24px', 
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ backgroundColor: '#333' }}>
                      <th style={{ padding: '0.5em', textAlign: 'left', border: '1px solid #dee2e6' }}>Character</th>
                      <th style={{ padding: '0.5em', textAlign: 'left', border: '1px solid #dee2e6' }}>Corporation</th>
                      <th style={{ padding: '0.5em', textAlign: 'left', border: '1px solid #dee2e6' }}>Alliance</th>
                      <th style={{ padding: '0.5em', textAlign: 'left', border: '1px solid #dee2e6' }}>Ship</th>
                      <th style={{ padding: '0.5em', textAlign: 'left', border: '1px solid #dee2e6' }}>Weapon</th>
                      <th style={{ padding: '0.5em', textAlign: 'left', border: '1px solid #dee2e6' }}>Damage Done</th>
                      <th style={{ padding: '0.5em', textAlign: 'left', border: '1px solid #dee2e6' }}>Final Blow</th>
                      <th style={{ padding: '0.5em', textAlign: 'left', border: '1px solid #dee2e6' }}>Security Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {killmailData.attackers.map((attacker, index) => (
                      <tr key={index} style={{ border: '1px solid #dee2e6' }}>
                        <td style={{ padding: '0.5em', border: '1px solid #dee2e6', color: 'white' }}>
                          {attacker.character_name || attacker.character_id || 'N/A'}
                          {attacker.character_id && (
                            <span style={{ fontSize: '0.9em' }}> (ID: {attacker.character_id})</span>
                          )}
                        </td>
                        <td style={{ padding: '0.5em', border: '1px solid #dee2e6', color: 'white' }}>
                          {attacker.corporation_name || attacker.corporation_id || 'N/A'}
                          {attacker.corporation_id && (
                            <span style={{ fontSize: '0.9em' }}> (ID: {attacker.corporation_id})</span>
                          )}
                        </td>
                        <td style={{ padding: '0.5em', border: '1px solid #dee2e6', color: 'white' }}>
                        ...
                        <td style={{ padding: '0.5em', border: '1px solid #dee2e6', color: 'white' }}>
                          ...
                          {attacker.ship_type_id && (
                            <span style={{ fontSize: '0.9em' }}> (ID: {attacker.ship_type_id})</span>
                          )}
                        </td>
                        <td style={{ padding: '0.5em', border: '1px solid #dee2e6', color: 'white' }}>
                          {attacker.weapon_name || attacker.weapon_type_id || 'N/A'}
                          {attacker.weapon_type_id && (
                            <span style={{ fontSize: '0.9em' }}> (ID: {attacker.weapon_type_id})</span>
                          )}
                        </td>
                        <td style={{ padding: '0.5em', border: '1px solid #dee2e6', color: 'white' }}>{attacker.damage_done.toLocaleString()}</td>
                        ...
                        <td style={{ padding: '0.5em', border: '1px solid #dee2e6', color: 'white' }}>{attacker.security_status.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div style={{ 
              ...
              {(() => {
                ...
                return (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2em' }}>
                    <div>
                                              <h5 style={{ margin: '0 0 1em 0', color: 'white', borderBottom: '2px solid #28a745', paddingBottom: '0.5em' }}>
                        ...
                        <div style={{ 
                          ...
                          {droppedItems.map((item, index) => (
                            <div key={index} style={{ 
                              ...
                              alignItems: 'center',
                              padding: '0.5em 0', 
                              borderBottom: index < droppedItems.length - 1 ? '1px solid #e8f5e8' : 'none',
                              fontSize: '0.9em'
                            }}>
                              <img 
                                src={`https://images.evetech.net/types/${item.typeId || 0}/icon?size=32`}
                                style={{
                                  width: '24px',
                                  height: '24px',
                                  marginRight: '0.5em',
                                ...
                                onError={(e) => {
                                  ...
                                  e.target.src = `https://images.evetech.net/types/${item.typeId || 0}/bp?size=32`;
                                }}
                              />
                    ...
                    <div>
                                              <h5 style={{ margin: '0 0 1em 0', color: 'white', borderBottom: '2px solid #dc3545', paddingBottom: '0.5em' }}>
                        ...
                        <div style={{ 
                          ...
                          {allDestroyedItems.map((item, index) => (
                            <div key={index} style={{ 
                              ...
                              alignItems: 'center',
                              padding: '0.5em 0', 
                              borderBottom: index < allDestroyedItems.length - 1 ? '1px solid #f5e8e8' : 'none',
                              fontSize: '0.9em'
                            }}>
                              <img 
                                src={`https://images.evetech.net/types/${item.typeId || 0}/icon?size=32`}
                  ...
                  <div style={{ marginBottom: '1em' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '0.9em', color: '#6c757d', marginTop: '0.5em' }}>
                        Items Appraised: {appraisalResult.item_count} | Based on Jita 4-4 market prices
                      </div>
                      ...
                      <div style={{ 
                        ...
                        {appraisalResult.items?.map((item, index) => (
                          <div key={index} style={{ 
                            ...
                            alignItems: 'center',
                            padding: '0.75em 1em',
                            borderBottom: index < appraisalResult.items.length - 1 ? '1px solid #eee' : 'none',
                            backgroundColor: index % 2 === 0 ? '#f8f9fa' : 'white'
                          }}>
                            <div style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                              <img 
                                src={`https://images.evetech.net/types/${item.itemType?.id || 0}/icon?size=32`}
                                style={{
                                  width: '32px',
                                  height: '32px',
                                  marginRight: '0.75em',
                                ...
                                onError={(e) => {
                                  e.target.onerror = null;
                                  e.target.src = `https://images.evetech.net/types/${item.itemType?.id || 0}/bp?size=32`;
                                }}
                              />
                              <div>
                                ...
                                <div style={{ fontSize: '0.85em', color: '#495057' }}>
                                  Quantity: {item.amount} | Volume: {item.totalVolume?.toLocaleString() || '0'} m³
                                </div>
                              </div>
                            </div>
        ...
        <Box>
          ...
          <div style={{ padding: "1em" }}>
            <p style={{ marginBottom: "1em" }}>
              Set the payout amount for this SRP report:
            </p>
            <div style={{ marginBottom: "1em" }}>
