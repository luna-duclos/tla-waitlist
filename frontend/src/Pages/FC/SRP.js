    ...
    <>
      ...
      <Box>
        ...
        <div style={{ display: "flex", gap: "1em", marginBottom: "1em" }}>
          ...
          <Button 
            onClick={handleTestCharacterWindow}
        ...
        {testWindowResult && (
          ...
          >
            <strong>{testWindowResult.success ? "Success:" : "Error:"}</strong> {testWindowResult.message}
          </div>
        )}
      </Box>
      ...
      <Box>
        ...
        {allStatuses && allStatuses.statuses && allStatuses.statuses.length > 0 && (
          <div style={{ marginBottom: "1em" }}>
            <p><strong>Total characters with active SRP coverage:</strong> {allStatuses.statuses.filter(payment => payment.expires_at * 1000 > Date.now()).length}</p>
          </div>
        )}
        {allStatuses && allStatuses.statuses && allStatuses.statuses.length > 0 ? (
          <div>
            <Table fullWidth>
              ...
              <TableBody>
                {allStatuses.statuses.slice(0, isExpanded ? allStatuses.statuses.length : 5).map((payment) => (
                  <Row key={payment.id}>
            ...
            {allStatuses.statuses.length > 5 && (
              <div style={{ marginTop: "1em", textAlign: "center" }}>
                ...
                >
                  {isExpanded ? `Show Less (5 of ${allStatuses.statuses.length})` : `Show More (${allStatuses.statuses.length - 5} more)`}
                </Button>
              </div>
            )}
          </div>
      ...
      <Box>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1em" }}>
          ...
          {!isAdminPage && (
            <Button
              onClick={() => window.location.href = "/fc/srp/submit"}
              style={{
        ...
        {srpReports && srpReports.reports && srpReports.reports.length > 0 ? (
            ...
            <TableBody>
              {srpReports.reports.map((report) => (
                <Row key={report.killmail_id}>
                      ...
                      onClick={() => {
                        ...
                        navigator.clipboard.writeText(formattedText).then(() => {
                          ...
                          if (window.showNotification) {
                            window.showNotification("Killmail link copied to clipboard!", "success");
                          } else {
                  ...
                  <Cell><TruncatedDescription text={report.description} /></Cell>
                  <Cell>
                  ...
                  {isAdminPage && (
                    <Cell style={{ verticalAlign: "middle" }}>
                      <Button
                        onClick={() => window.location.href = `/srp-report-detail?id=${report.killmail_id}`}
                        style={{
                          padding: "0.25em 0.5em",
                          fontSize: "0.875em",
                          backgroundColor: "#007bff",
                  ...
                  {!isAdminPage && report.status === "pending" ? (
                    <Cell style={{ verticalAlign: "middle" }}>
                      <Button
                        onClick={() => window.location.href = `/fc/srp/submit?edit=${report.killmail_id}`}
                        style={{
                          padding: "0.25em 0.5em",
                          fontSize: "0.875em",
          ...
          <div style={{ padding: "1em", backgroundColor: "#f8f9fa", borderRadius: "4px", textAlign: "center" }}>
            <p><strong>No SRP reports found.</strong></p>
